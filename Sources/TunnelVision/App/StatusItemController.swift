...
final class StatusItemController: NSObject {
    ...
    /// Quick action "Adjust allowlist…": opens the picker for the task at hand.
    ...
    private func refreshLabel() {
        ...
        let image = NSImage(
            ...
            accessibilityDescription: model.phase == .breakTime ? "On break" : "Tunnel Vision timer"
        )
    ...
    private var tooltip: String {
        switch model.phase {
        case .idle: return "Tunnel Vision — click for your tasks"
        case .work:
    ...
    private func buildQuickMenu() -> NSMenu {
        let menu = NSMenu()
        menu.addItem(makeItem("New task…", #selector(quickNewTask)))
        ...
        if model.phase == .work || model.phase == .paused {
            for minutes in [5, 10] {
                let item = NSMenuItem(title: "Add \(minutes) minutes", action: #selector(quickExtend(_:)), keyEquivalent: "")
                item.target = self
                item.representedObject = minutes
                menu.addItem(item)
            }
            menu.addItem(NSMenuItem.separator())
            menu.addItem(makeItem("Adjust allowlist…", #selector(quickOpenPicker)))
        ...
        menu.addItem(NSMenuItem.separator())
        menu.addItem(makeItem("Quit Tunnel Vision", #selector(quickQuit)))
        return menu
    }
    ...
    @objc private func quickNewTask() {
        openNewTask()
    }

    @objc private func quickOpenPicker() {
        if attachedSheet == nil {
            popover.performClose(nil)
        }
        onOpenPicker?()
    ...
    /// Opens the panel with the new-task sheet up (quick action and global
    /// shortcut). Mid-edit, the existing sheet comes forward instead of a
    /// second one.
    func openNewTask() {
        if let sheet = attachedSheet {
            NSApp.activate(ignoringOtherApps: true)
    ...
    @objc private func quickExtend(_ sender: NSMenuItem) {
