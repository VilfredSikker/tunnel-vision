...
struct PickerWindowInfo: Identifiable, Hashable, Sendable {
    let id: CGWindowID
    let appPID: pid_t
    /// Nil when no title source is granted (or the window is untitled).
    let title: String?
    /// Screen bounds (top-left origin), for matching against browser scripting.
    let bounds: CGRect?
    /// The active tab's URL for browser windows, once the browser answered.
    var url: String?

    init(id: CGWindowID, appPID: pid_t, title: String?, bounds: CGRect? = nil, url: String? = nil) {
        self.id = id
        self.appPID = appPID
        self.title = title
        self.bounds = bounds
    ...
    /// What the tile shows and what a pick is matched by: the title, else
    /// the URL trimmed to host and path.
    var displayName: String? {
        title ?? url.flatMap(Self.pattern(fromURL:))
    }

    /// "https://github.com/org/repo/pull/1?x=1#y" → "github.com/org/repo/pull/1".
    /// Only web URLs qualify: `chrome://newtab` or `about:blank` name nothing.
    static func pattern(fromURL raw: String) -> String? {
    ...
    nonisolated static func assemble(
        ...
        for window in windows {
            ...
            windowsByPID[window.ownerPID, default: []].append(
                PickerWindowInfo(id: window.id, appPID: window.ownerPID, title: title, bounds: window.bounds)
            )
        }
...
/// A single window picked by URL when the browser named one, else by title.
struct PickedWindowRef: Hashable, Sendable {
    let bundleID: String
    let windowID: CGWindowID
    let title: String?
    let url: String?
    /// A URL pick widened to the whole site (host only, no path).
    let siteWide: Bool

    init(bundleID: String, windowID: CGWindowID, title: String?, url: String? = nil, siteWide: Bool = false) {
...
/// What existing rules pre-select in the picker.
struct PickerSeed: Equatable, Sendable {
    var wholeAppBundles: Set<String> = []
    var windowIDs: Set<CGWindowID> = []
    /// Windows matched by a site-wide URL rule (host only).
    var siteWideWindowIDs: Set<CGWindowID> = []
    var herdrLabels: Set<String> = []
}

/// A herdr workspace picked by label, with the terminal that hosts herdr.
struct PickedHerdrRef: Hashable, Sendable {
    /// Nil when the hosting terminal could not be identified.
...
enum SelectionBuilder {
    ...
    /// herdr picks always become rules — even inside a whole-app pick they
    /// narrow what the terminal may show.
    static func rules(
        wholeAppBundles: Set<String>,
        windows: [PickedWindowRef],
        herdr: [PickedHerdrRef] = []
    ) -> [Rule] {
        var rules: [Rule] = wholeAppBundles.sorted().map { Rule(bundleID: $0, scope: .app) }
        for window in windows where !wholeAppBundles.contains(window.bundleID) {
            // A window pick implies its app at layer 1; the pattern is what
            // the window and browser layers enforce inside it.
            if let url = window.url, let pattern = PickerWindowInfo.pattern(fromURL: url) {
                // A browser window is known by its URL: a tab switch changes
                // the title, so the site rule is the stable handle. Site-wide
                // picks keep the host only.
                let sitePattern = window.siteWide ? (URLPattern.site(fromURL: url) ?? pattern) : pattern
                rules.append(Rule(bundleID: window.bundleID, scope: .url, pattern: sitePattern))
            } else if let title = window.title?.trimmingCharacters(in: .whitespaces), !title.isEmpty {
                rules.append(Rule(bundleID: window.bundleID, scope: .window, pattern: title))
            }
        }
        for pick in herdr.sorted(by: { $0.label.localizedCaseInsensitiveCompare($1.label) == .orderedAscending }) {
            let label = pick.label.trimmingCharacters(in: .whitespaces)
            guard !label.isEmpty else { continue }
            // The host bundle rides along so the app-level lock keeps the
            // terminal open; the label is what the herdr guard matches.
            rules.append(Rule(bundleID: pick.hostBundleID ?? "", scope: .herdr, pattern: label))
        }
        return rules
    }

    /// Seed state from existing rules: app-scope rules mark their bundle;
    /// window-scope rules mark matching windows when present; herdr rules
    /// mark their labels (lowercased).
    static func seed(
        apps: [PickerAppInfo],
        rules: [Rule]
    ) -> PickerSeed {
        ...
        for rule in rules where rule.effect == .allow {
            switch rule.scope {
            case .app:
                seed.wholeAppBundles.insert(rule.bundleID)
            case .window:
                ...
                for app in apps where app.bundleID == rule.bundleID {
                    for window in app.windows where (window.title?.lowercased() ?? "").contains(pattern) {
                        seed.windowIDs.insert(window.id)
                    }
                }
            case .url:
                // Marks the windows whose URL matches when the browser has
                // answered; otherwise the app stays allowed as a whole (a URL
                // rule admits its app at layer 1 either way).
                let matched = windowsMatching(urlRule: rule, in: apps)
                seed.windowIDs.formUnion(matched)
                if isSiteWide(rule.pattern) {
                    seed.siteWideWindowIDs.formUnion(matched)
                }
                if matched.isEmpty {
                    seed.wholeAppBundles.insert(rule.bundleID)
                }
    ...
    /// Windows of the rule's app whose URL the rule's pattern matches.
    static func windowsMatching(urlRule rule: Rule, in apps: [PickerAppInfo]) -> Set<CGWindowID> {
        guard URLPattern.parse(rule.pattern) != nil else { return [] }
        var matched = Set<CGWindowID>()
        for app in apps where app.bundleID == rule.bundleID {
            for window in app.windows {
                if let url = window.url, URLPattern.matches(rule.pattern, url: url) {
                    matched.insert(window.id)
                }
            }
        }
        return matched
    ...
    /// A URL pattern without a path covers the whole site.
    static func isSiteWide(_ pattern: String) -> Bool {
        guard let parts = URLPattern.parse(pattern) else { return false }
        return parts.path.isEmpty
    }
}
