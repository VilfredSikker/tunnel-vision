...
final class FakeProcessManager: ProcessManaging {
    ...
    /// Every hide request, including repeats and lost ones.
    var hideCalls: [pid_t] = []
    /// PIDs whose next hide is lost, the way a Dock-click unhide landing
    /// after our hide leaves the app on screen.
    var swallowNextHide: Set<pid_t> = []
    ...
    func isHidden(pid: pid_t) -> Bool {
        hidden.contains(pid) && !surfaced.contains(pid)
    }
...
final class EnforcerTests: XCTestCase {
    ...
    override func setUp() async throws {
        thawURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("TunnelVisionEnforcer-\(UUID().uuidString)")
    ...
    func testDarkReassertsHideWhenTheAppSurfacesAgain() {
        let fake = FakeProcessManager()
        fake.launch(bundleID: slackID, name: "Slack", pid: 102)
        let enforcer = AppEnforcer(process: fake, frozenStore: store)
        enforcer.lock(mode: .dark, rules: [Rule(bundleID: xcodeID)])
        XCTAssertEqual(fake.hideCalls, [102])
        XCTAssertFalse(enforcer.reassertHidden(pid: 102), "hidden as intended: nothing to re-send")

        // Dock click: the Dock unhides Slack, the activation is enforced, but
        // the Dock's unhide lands after our hide, so Slack is back on screen.
        fake.surfaced.insert(102)
        fake.swallowNextHide = [102]
        enforcer.enforce(snapshot: fake.apps[0])
        XCTAssertEqual(fake.hideCalls, [102, 102])
        XCTAssertFalse(fake.isHidden(pid: 102), "the raced hide was lost")

        XCTAssertTrue(enforcer.reassertHidden(pid: 102), "the follow-up check hides it again")
        XCTAssertEqual(fake.hideCalls, [102, 102, 102])
        XCTAssertTrue(fake.isHidden(pid: 102))
        XCTAssertFalse(enforcer.reassertHidden(pid: 102))
        enforcer.unlock()
    }

    func testReassertHiddenLeavesAllowedAndUnlockedAppsAlone() {
        let fake = FakeProcessManager()
        fake.launch(bundleID: slackID, name: "Slack", pid: 102)
        let enforcer = AppEnforcer(process: fake, frozenStore: store)
        enforcer.lock(mode: .dark, rules: [Rule(bundleID: xcodeID)])
        ...
        XCTAssertFalse(fake.isHidden(pid: 102), "allow-for-session brought Slack back")
        XCTAssertFalse(enforcer.reassertHidden(pid: 102), "an allowed app is never re-hidden by a stale recheck")
        XCTAssertEqual(fake.hideCalls, [102])

        enforcer.unlock()
        XCTAssertFalse(enforcer.reassertHidden(pid: 102), "no session, no enforcement")
        XCTAssertEqual(fake.hideCalls, [102])
    }

    func testFrozenVictimsAreNotReHidden() {
        let fake = FakeProcessManager()
        fake.launch(bundleID: slackID, name: "Slack", pid: 102)
        let enforcer = AppEnforcer(process: fake, frozenStore: store)
        enforcer.lock(mode: .frozen, rules: [Rule(bundleID: xcodeID)])
        fake.surfaced.insert(102) // the hide never took
        XCTAssertFalse(enforcer.reassertHidden(pid: 102), "a stopped process cannot act on a hide anyway")
        enforcer.unlock()
    }
    ...
    func testThawsFrozenAppsFromPreviousRun() {
        ...
        let enforcer = AppEnforcer(process: fake, frozenStore: store)
        XCTAssertTrue(fake.resumed.contains(777), "Tunnel Vision died frozen: next launch SIGCONTs survivors")
